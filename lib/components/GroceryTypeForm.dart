...
void AddGroceryTypeForm(BuildContext context){
...
void AddHandler(BuildContext cxt,StateSetter setState) async{
   ...
   final typeProvider = Provider.of<GroceryTypesProvider>(context, listen: false);
...
setState((){});
 ...
 try{
      dynamic data = {
        "type_name": textInputController["name"]?.value?.text
      };
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.post(Uri.parse("${ApiServices.get_grocery_type}"), body: data);
...
setState((){});
     ...
     if(response.statusCode == 200){
        ...
        Types type = Types.fromJson(response_body);
        print(response_body);
        typeProvider.addNew(type);
        CustomSnackBar().ErrorMsgSnackBar("Type Added");
...
void UpdateGroceryTypeForm(BuildContext context, Types type){
   double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
...
late final Map<String, TextEditingController> textInputController = {
  "name": TextEditingController(text: type?.typeName),
  };
...
void updateHandler(BuildContext cxt,StateSetter setState) async{
   final SharedPreferences prefs = await SharedPreferences.getInstance();
  final typeProvider = Provider.of<GroceryTypesProvider>(cxt, listen: false);
...
setState((){});
 ...
 try{
      type.typeName = textInputController["name"]?.value?.text;

      final data = json.encode(type);
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.put(Uri.parse("${ApiServices.get_grocery_type}"), body: data, headers: header);
    print(response.statusCode);
...
setState((){});
     ...
     if(response.statusCode == 200){
        ...
        Types type = Types.fromJson(response_body);
        print(response_body);
        typeProvider.Update(type);
        CustomSnackBar().ErrorMsgSnackBar("Type Updated");
  Navigator.of(cxt).pop();
