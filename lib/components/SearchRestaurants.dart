...
void searchrestaurants(BuildContext context, Future<void> Function(List<RestaurantModel> restaurants) additemHandler){
  double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
bool loading = false;
final SelectedProvider = Provider.of<SearchRestaurantsProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.searchrestaurants}${text}";
     print(url);
...
myState(() {
    ...
    if(response.statusCode == 200){
      ...
      _list.add(RestaurantModel.fromJson(json));
       }
     SelectedProvider.addrestaurants(_list);
            ...
            return Dialog(
              ...
              child: StatefulBuilder(
                builder: (context, myState) {
                  final SelectedProvider = Provider.of<SearchRestaurantsProvider>(context, listen: true);
final SelectedRestaurants = SelectedProvider.SelectedRestaurants;
 final restaurants = SelectedProvider.restaurants;

                  return Scaffold(
                    bottomNavigationBar: (SelectedRestaurants?.length ?? 0) > 0 ?
                    InkWell( 
            onTap: () async{
              loading = true;
              myState(() => {});

              await additemHandler(SelectedRestaurants);
  ...
  if(loading)
                          ...
                          minHeight: 10,
                           ...
                           if((SelectedRestaurants?.length ?? 0) > 0)
                                    InkWell(
                                      onTap: (){
                                       SelectedRestaurantsDialog(context);
                                      },
                                      child: Container(
                                       margin: const EdgeInsets.only(left: 20),
                                        ...
                                        padding: const EdgeInsets.all(10),
                                              ...
                                              child: Text("${SelectedRestaurants?.length ?? 0}",
                             ...
                             return Center(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  SizedBox(width: 10,),
                                  Text(data?[index].restaurantName ?? '', style: TextStyle(color: AppColors.blackcolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w400),)
                                ],
                              )
                             ),
                                  
                                                 if((SelectedRestaurants?.where((e) => e?.restaurantId == data?[index]?.restaurantId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
...
final SelectedProvider = Provider.of<SearchRestaurantsProvider>(context, listen: false);
SelectedProvider.addSelectedrestaurants([]);
SelectedProvider.addrestaurants([]);
     });
}



void SelectedRestaurantsDialog(BuildContext context){
  double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
                    ...
                    body: Column(
                      children: [
                         Expanded(
                           child: ListView.builder(
                            shrinkWrap: true,
                            itemCount: SelectedRestaurants?.length ?? 0,
                            padding: const EdgeInsets.symmetric(horizontal: 5),
                            itemBuilder: (BuildContext context, int index){
                              final data = SelectedRestaurants;
                             return Center(
                               child: InkWell(
                                onTap: (){
