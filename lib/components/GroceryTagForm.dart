...
void AddGrocerytagForm(BuildContext context){
...
setState((){});
 ...
 try{
      dynamic data = {
        "tag_name": textInputController["name"]?.value?.text
      };
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.post(Uri.parse("${ApiServices.get_grocery_tag}"), body: data);
...
setState((){});
     ...
     if(response.statusCode == 200){
        ...
        GroceryTagModel tag = GroceryTagModel.fromJson(response_body);
        print(response_body);
        tagProvider.addNew(tag);
...
void UpdateGroceryTagForm(BuildContext context, GroceryTagModel tag){
   double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
...
void updateHandler(BuildContext cxt,StateSetter setState) async{
   final SharedPreferences prefs = await SharedPreferences.getInstance();
   final tagProvider = Provider.of<GroceryTagProvider>(cxt, listen: false);
...
setState((){});
 ...
 try{
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.put(Uri.parse("${ApiServices.get_grocery_tag}"), body: data, headers: header);
    print(response.statusCode);
