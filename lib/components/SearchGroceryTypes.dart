...
void SearchGroceryTypes(BuildContext context, Future<void> Function(List<Types> types) additemHandler){
...
try{
       String url = "${ApiServices.search_grocery_types}${text}";
     print(url);
...
myState(() {
    ...
    if(response.statusCode == 200){
     ...
     SelectedProvider.addTypes(_list);
            ...
            return Dialog(
              ...
              child: StatefulBuilder(
                builder: (context, myState) {
                  final SelectedProvider = Provider.of<SearchGroceryTypesProvider>(context, listen: true);
final SelectedTypes = SelectedProvider.SelectedTypes;
 final types = SelectedProvider.types;

                  return Scaffold(
                    bottomNavigationBar: (SelectedTypes?.length ?? 0) > 0 ?
                    InkWell( 
            onTap: () async{
              loading = true;
              myState(() => {});

              await additemHandler(SelectedTypes);
                                      ...
                                      child: Container(
                                       margin: const EdgeInsets.only(left: 20),
                                        ...
                                        padding: const EdgeInsets.all(10),
                                              ...
                                              child: Text("${SelectedTypes?.length ?? 0}",
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text(data?[index].typeName ?? '', style: TextStyle(color: AppColors.blackcolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w400),)
                                                 ...
                                                 if((SelectedTypes?.where((e) => e?.typeId == data?[index]?.typeId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
...
final SelectedProvider = Provider.of<SearchGroceryTypesProvider>(context, listen: false);
SelectedProvider.addTypes([]);
SelectedProvider.addSelectedTypes([]);
     });
}
...
void SelectedDishesDialog(BuildContext context){
  ...
  final SelectedProvider = Provider.of<SearchGroceryTypesProvider>(context, listen: true);
final types = SelectedProvider.SelectedTypes;
                  return Scaffold(
                    backgroundColor: AppColors.whitecolor,
                     appBar: AppBar(
                         ...
                         Expanded(
                           child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index){
                             ...
                             )  ,            if((types?.where((e) => e?.typeId == data?[index]?.typeId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
                                                    child: Icon(Icons.check_box, color: AppColors.primaryColor, size: ScreenUtil().setSp(20),)),
