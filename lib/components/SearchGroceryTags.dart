...
void SearchGroceryTags(BuildContext context, Future<void> Function(List<GroceryTagModel> tags) additemHandler){
  double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
bool loading = false;
final SelectedProvider = Provider.of<SearchGroceryTagsProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.search_grocery_tags}${text}";
     print(url);
...
myState(() {
    ...
    if(response.statusCode == 200){
     ...
     SelectedProvider.addTags(_list);
            ...
            return Dialog(
              ...
              child: StatefulBuilder(
                builder: (context, myState) {
                  final SelectedProvider = Provider.of<SearchGroceryTagsProvider>(context, listen: true);
final SelectedTags = SelectedProvider.SelectedTags;
 final tags = SelectedProvider.tags;

                  return Scaffold(
                    bottomNavigationBar: (SelectedTags?.length ?? 0) > 0 ?
                    InkWell( 
            onTap: () async{
              loading = true;
              myState(() => {});

              await additemHandler(SelectedTags);
                                      ...
                                      child: Container(
                                       margin: const EdgeInsets.only(left: 20),
                                        ...
                                        padding: const EdgeInsets.all(10),
                                              ...
                                              child: Text("${SelectedTags?.length ?? 0}",
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text(data?[index].tagName ?? '', style: TextStyle(color: AppColors.blackcolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w400),)
                                                 ...
                                                 if((SelectedTags?.where((e) => e?.tagId == data?[index]?.tagId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
...
final SelectedProvider = Provider.of<SearchGroceryTagsProvider>(context, listen: false);
SelectedProvider.addTags([]);
SelectedProvider.addSelectedTags([]);
     });
}



void SelectedTagsDialog(BuildContext context){
  double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
  ...
  final SelectedProvider = Provider.of<SearchGroceryTagsProvider>(context, listen: true);
final tags = SelectedProvider.SelectedTags;
                  return Scaffold(
                    backgroundColor: AppColors.whitecolor,
                     appBar: AppBar(
                         ...
                         Expanded(
                           child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index){
                             ...
                             )  ,            if((tags?.where((e) => e?.tagId == data?[index]?.tagId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
                                                    child: Icon(Icons.check_box, color: AppColors.primaryColor, size: ScreenUtil().setSp(20),)),
