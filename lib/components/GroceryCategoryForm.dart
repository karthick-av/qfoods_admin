...
void AddGroceryCategoryForm(BuildContext context){
...
setState((){});
 ...
 try{
      dynamic data = {
        "category_name": textInputController["name"]?.value?.text
      };
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.post(Uri.parse("${ApiServices.get_grocery_categories}"), body: data);
...
setState((){});
     ...
     if(response.statusCode == 200){
        ...
        Category cat = Category.fromJson(response_body);
        print(response_body);
        categoryProvider.addNew(cat);
...
void UpdateGroceryCategoryForm(BuildContext context, Category catgeory){
   double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
...
void updateHandler(BuildContext cxt,StateSetter setState) async{
   final SharedPreferences prefs = await SharedPreferences.getInstance();
  final categoryProvider = Provider.of<GrocerycategoryProvider>(cxt, listen: false);
...
setState((){});
 ...
 print(ApiServices.get_categories);
    ...
    var response = await http.put(Uri.parse("${ApiServices.get_grocery_categories}"), body: data, headers: header);
    print(response.statusCode);
