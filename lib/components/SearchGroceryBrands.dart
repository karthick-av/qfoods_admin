...
void SearchGroceryBrands(BuildContext context, Future<void> Function(List<Brands> brands) additemHandler){
...
try{
       String url = "${ApiServices.search_grocery_brands}${text}";
     print(url);
...
myState(() {
    ...
    if(response.statusCode == 200){
     ...
     SelectedProvider.addBrands(_list);
            ...
            return Dialog(
              ...
              child: StatefulBuilder(
                builder: (context, myState) {
                  final SelectedProvider = Provider.of<SearchGroceryBrandsProvider>(context, listen: true);
final SelectedBrands = SelectedProvider.SelectedBrands;
 final brands = SelectedProvider.brands;

                  return Scaffold(
                    bottomNavigationBar: (SelectedBrands?.length ?? 0) > 0 ?
                    InkWell( 
            onTap: () async{
              loading = true;
              myState(() => {});

              await additemHandler(SelectedBrands);
          ...
          //  UpdateMenuHandler();
            ...
            child: Text("Add", style: TextStyle(color: AppColors.whitecolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w500),),
                    ...
                    backgroundColor: AppColors.whitecolor,
                     appBar: AppBar(
                      title: const Text('Search Brands',
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  Text(data?[index].brandName ?? '', style: TextStyle(color: AppColors.blackcolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w400),)
                                                 ...
                                                 if((SelectedBrands?.where((e) => e?.brandId == data?[index]?.brandId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
...
final SelectedProvider = Provider.of<SearchGroceryBrandsProvider>(context, listen: false);
SelectedProvider.addBrands([]);
SelectedProvider.addSelectedbrands([]);
     });
}
...
void SelectedDishesDialog(BuildContext context){
  ...
  final SelectedProvider = Provider.of<SearchGroceryBrandsProvider>(context, listen: true);
final brands = SelectedProvider.SelectedBrands;
                  return Scaffold(
                    backgroundColor: AppColors.whitecolor,
                     appBar: AppBar(
                         ...
                         Expanded(
                           child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index){
                             ...
                             )  ,            if((brands?.where((e) => e?.brandId == data?[index]?.brandId)?.length ?? 0) > 0)
                                                  Positioned(
                                                    left: 0,
                                                    top: 10,
                                                    child: Icon(Icons.check_box, color: AppColors.primaryColor, size: ScreenUtil().setSp(20),)),
