...
void AddGroceryBrandForm(BuildContext context){
...
setState((){});
 ...
 try{
      dynamic data = {
        "brand_name": textInputController["name"]?.value?.text
      };
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.post(Uri.parse("${ApiServices.get_grocery_brand}"), body: data);
...
setState((){});
     ...
     if(response.statusCode == 200){
        ...
        Brands brand = Brands.fromJson(response_body);
        print(response_body);
        brandsProvider.addNew(brand);
        CustomSnackBar().ErrorMsgSnackBar("brand Added");
...
void UpdateGroceryBrandForm(BuildContext context, Brands brand){
   double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
...
late final Map<String, TextEditingController> textInputController = {
  "name": TextEditingController(text: brand?.brandName),
  };
...
void updateHandler(BuildContext cxt,StateSetter setState) async{
   final SharedPreferences prefs = await SharedPreferences.getInstance();
  final brandProvider = Provider.of<GroceryBrandsProvider>(cxt, listen: false);
...
setState((){});
 ...
 try{
      brand.brandName = textInputController["name"]?.value?.text;

      final data = json.encode(brand);
  ...
  'Content-type': 'application/json'
    ...
    var response = await http.put(Uri.parse("${ApiServices.get_grocery_brand}"), body: data, headers: header);
    print(response.statusCode);
...
setState((){});
     ...
     if(response.statusCode == 200){
        ...
        Brands brand = Brands.fromJson(response_body);
        print(response_body);
        brandProvider.Update(brand);
        CustomSnackBar().ErrorMsgSnackBar("Brand Updated");
  Navigator.of(cxt).pop();
