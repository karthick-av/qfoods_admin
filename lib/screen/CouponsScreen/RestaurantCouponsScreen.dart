...
import 'package:admin/screen/CouponsScreen/ViewRestaurantCouponScreen.dart';
...
class RestaurantCouponsScreen extends StatefulWidget {
  ...
  @override
  State<RestaurantCouponsScreen> createState() => _RestaurantCouponsScreenState();
}
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((list?.restaurant?.length ?? 0) == 0 && list.apiResCalled == false){
      getRestaurantCouponHandler("init", "");
     ...
     scrollController.addListener(() { 
      ...
      print(ApiCallDone);
       ...
       if(!ApiCallDone){
       getRestaurantCouponHandler("bottom",filterCondition);
...
Future<void> getRestaurantCouponHandler(String type, String filter)async {
...
try{
       String url = "${ApiServices.coupon}?type=1&page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
setState(() {
    ...
    if(response.statusCode == 200){
       ...
       for(var json in response_body){
            _list.add(CouponModel.fromJson(json));
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      list.addAllRestaurant(_list);   
      }else{
       list.addRestaurant(_list);   
...
filterCondition = val;
setState(() {});
getRestaurantCouponHandler("init", val);
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: RefreshIndicator(
             onRefresh: () async{
              ResetState();
                await getRestaurantCouponHandler("init", "");
           ...
           padding: const EdgeInsets.symmetric(horizontal: 15,vertical: 10),
                           ...
                           child: TextField(
                            ...
                            ResetState();
                await getRestaurantCouponHandler("init", "search=${val}");
                    ...
                    child: Column(
                      children: [
                         ...
                         ListView.builder(
                          ...
                          itemCount: list?.restaurant?.length ?? 0,
                          itemBuilder: (BuildContext context, int index){
                            final data = list?.restaurant;
                           return InkWell(
                            onTap: (){
                               Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => 
                        ViewRestaurantCouponScreen(coupon: data![index],)),
                        );
                            },
                             child: Container(
