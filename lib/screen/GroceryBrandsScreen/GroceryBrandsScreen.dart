...
import 'package:admin/components/BrandForm.dart';
...
class GroceryBrandScreen extends StatefulWidget {
  const GroceryBrandScreen({super.key});

  @override
  State<GroceryBrandScreen> createState() => _GroceryBrandScreenState();
}

class _GroceryBrandScreenState extends State<GroceryBrandScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
    final list = Provider.of<GroceryBrandsProvider>(context, listen: false);

   if((list?.list?.length ?? 0) == 0){
      getBrandsHandler("init", "");
     ...
     scrollController.addListener(() { 
      ...
      print(ApiCallDone);
       ...
       if(!ApiCallDone){
       getBrandsHandler("bottom",filterCondition);
...
Future<void> getBrandsHandler(String type, String filter)async {
  if(CompleteAPI) return;
final brandsProvider = Provider.of<GroceryBrandsProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.get_grocery_brand}?page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
setState(() {
    ...
    if(response.statusCode == 200){
       ...
       for(var json in response_body){
            _list.add(Brands.fromJson(json));
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      brandsProvider.addAll(_list);   
      }else{
       brandsProvider.add(_list);   
...
Future<void> deleteHandler(int id) async{
 loading = true;
  ...
  try{
      ...
      var response = await http.delete(Uri.parse("${ApiServices.get_grocery_brand}/${id}"));
  loading = false;
    setState(() {});
    
   if(response.statusCode == 200){
    brandsProvider.remove(id);
  }
  }
  catch(e){
      loading = false;
...
getBrandsHandler("init", val);
  ...
  Widget build(BuildContext context) {
    final brandsProvider = Provider.of<GroceryBrandsProvider>(context, listen: true);
      ...
      body: RefreshIndicator(
           ...
           padding: const EdgeInsets.symmetric(horizontal: 15,vertical: 10),
                           ...
                           child: TextField(
                            ...
                            ResetState();
                await getBrandsHandler("init", "search=${val}");
                    ...
                    child: Column(
                      children: [
                         ...
                         ListView.builder(
                          ...
                          itemCount: brandsProvider?.list?.length ?? 0,
                          itemBuilder: (BuildContext context, int index){
                            final data = brandsProvider?.list;
                           return InkWell(
                            onTap: (){
              Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => 
                        ViewBrandProductsScreen(brand: data![index],)),
                        );
                            },
                             child: Container(
...
children:  [
      ...
      SlidableAction(
        onPressed: ((context) {
          print(data![index]?.brandName);
          
        UpdateGroceryBrandForm(context, data![index]);
...
children:  [
      ...
      SlidableAction(
         onPressed: ((context) {
        deleteHandler(data?[index]?.brandId ?? 0);
        }),
        ...
        label: 'Delete',
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(width: 10,),
                                    Text(data?[index].brandName ?? '', style: TextStyle(color: AppColors.blackcolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w400),)
                                  ],
                                ),
                              )
                             ),
...
class GroceryBrandsProvider extends ChangeNotifier{
 List<Brands> list = [];
  addAll(List<Brands> data) {
    list = data;
    notifyListeners();
  }

  add(List<Brands> data) {
     list = [...list, ...data];
  ...
  addNew(Brands data) {
    int? index = list?.indexWhere((e) => (e?.brandId ?? 0) == data?.brandId);
    if(index == -1){
      list.insert(0, data);
    }
 ...
 Update(Brands data) {
    int index = list?.indexWhere((e) => (e?.brandId ?? 0) == data?.brandId) ?? -1;
    if(index != -1){
      list[index] = data;
    }
  ...
  remove(int id) {
   list.removeWhere((e) => e?.brandId == id);
  }
