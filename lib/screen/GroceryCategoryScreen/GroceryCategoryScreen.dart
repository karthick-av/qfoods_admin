...
import 'package:admin/components/GroceryCategoryForm.dart';
...
import 'package:admin/screen/GroceryCategoryScreen/ViewGroceryCategoryProducts.dart';
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
    final list = Provider.of<GrocerycategoryProvider>(context, listen: false);
...
final categoryProvider = Provider.of<GrocerycategoryProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.get_grocery_categories}?page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      categoryProvider.addAll(_list);   
      }else{
       categoryProvider.add(_list);   
...
getCategoriesHandler("init", val);
  ...
  Widget build(BuildContext context) {
     final categoryProvider = Provider.of<GrocerycategoryProvider>(context, listen: true);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          InkWell(
            onTap: (){
              AddGroceryCategoryForm(context);
            },
                    ...
                    child: Column(
                      children: [
                         ...
                         ListView.builder(
                          ...
                          itemCount: categoryProvider?.list?.length ?? 0,
                          itemBuilder: (BuildContext context, int index){
                            final data = categoryProvider?.list;
                           return InkWell(
                            onTap: (){
              Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => 
                        ViewCategoryProductsScreen(category: data![index],)),
                        );
                            },
                             child: Container(
...
children:  [
      ...
      SlidableAction(
        onPressed: ((context) {
          print(data![index]?.categoryName);
          
        UpdateGroceryCategoryForm(context, data![index]);
...
class GrocerycategoryProvider extends ChangeNotifier{
 List<Category> list = [];
  addAll(List<Category> data) {
    list = data;
    notifyListeners();
  }

  add(List<Category> data) {
     list = [...list, ...data];

    notifyListeners();
  }

  addNew(Category data) {
