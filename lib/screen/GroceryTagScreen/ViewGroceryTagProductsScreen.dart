...
class ViewGroceryTagProductsScreen extends StatefulWidget {
  final GroceryTagModel tag;
  const ViewGroceryTagProductsScreen({super.key, required this.tag});

  @override
  State<ViewGroceryTagProductsScreen> createState() => _ViewGroceryTagProductsScreenState();
}

class _ViewGroceryTagProductsScreenState extends State<ViewGroceryTagProductsScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   final productsprovider = Provider.of<GroceryTagProductsProvider>(context, listen: false);
   if((productsprovider?.list?.length ?? 0) == 0){
      getTagProducstHandler("init", "");
     ...
     scrollController.addListener(() { 
      ...
      print(ApiCallDone);
       ...
       if(!ApiCallDone){
       getTagProducstHandler("bottom",filterCondition);
...
Future<void> getTagProducstHandler(String type, String filter)async {
   final productsProvider = Provider.of<GroceryTagProductsProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.get_grocery_tag}/products/${widget.tag?.tagId}?page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
filterCondition = val;
setState(() {});
getTagProducstHandler("init", val);
}
...
Future<void> AdditemHandler(List<SearchProductModel> product) async{
     try{
       String url = "${ApiServices.get_grocery_tag}/products/";
     List<dynamic> data = [];
     product?.forEach((e) {
      data.add({
        "product_id": e?.groceryId?.toString(),
        "tag_id":  widget?.tag?.tagId?.toString()
      });
     });
 var header ={
...
Future<void> deleteitemHandler(SearchProductModel product) async{
  final productsProvider = Provider.of<GroceryTagProductsProvider>(context, listen: false);
     try{
       String url = "${ApiServices.get_grocery_tag}/products/${widget?.tag?.tagId}/${product?.groceryId}";
   print(url);
 ...
 final productsProvider = Provider.of<GroceryTagProductsProvider>(context, listen: true);
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
             onRefresh: () async{
              ResetState();
                await getTagProducstHandler("init", "");
           ...
           padding: const EdgeInsets.symmetric(horizontal: 15,vertical: 10),
                           ...
                           child: TextField(
                            ...
                            ResetState();
                await getTagProducstHandler("init", "search=${val}");
...
class GroceryTagProductsProvider extends ChangeNotifier{
