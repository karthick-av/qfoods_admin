...
class GroceryOrdersDeliveredScreen extends StatefulWidget {
  const GroceryOrdersDeliveredScreen({super.key});

  @override
  State<GroceryOrdersDeliveredScreen> createState() => _GroceryOrdersDeliveredScreenState();
}

class _GroceryOrdersDeliveredScreenState extends State<GroceryOrdersDeliveredScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((orders?.delivered?.length ?? 0) == 0 && orders?.ldelivered == false){
...
try{
       String url = "${ApiServices.grocery_get_orders}?isDelivered=true&page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
    var response = await http.get(Uri.parse(url));

    orders.updateDelivered();
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orders.addAllDelivered(__orders);   
      }else{
       orders.addDelivered(__orders);   
...
getOrdersHandler("init", val);
  ...
  Widget build(BuildContext context) {
    ...
    final orders = Provider.of<GroceryOrdersProvider>(context, listen: true).delivered;
