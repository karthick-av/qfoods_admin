...
class GroceryOrdersOntheWayScreen extends StatefulWidget {
  const GroceryOrdersOntheWayScreen({super.key});

  @override
  State<GroceryOrdersOntheWayScreen> createState() => _GroceryOrdersOntheWayScreenState();
}

class _GroceryOrdersOntheWayScreenState extends State<GroceryOrdersOntheWayScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((orders?.ontheway?.length ?? 0) == 0 && orders?.lontheway == false){
...
try{
       String url = "${ApiServices.grocery_get_orders}?status=3&page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
    var response = await http.get(Uri.parse(url));

    orders.updateOntheWay();
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orders.addAllOntheway(__orders);   
      }else{
       orders.addOntheway(__orders);   
...
getOrdersHandler("init", val);
  ...
  Widget build(BuildContext context) {
    ...
    final orders = Provider.of<GroceryOrdersProvider>(context, listen: true).ontheway;
