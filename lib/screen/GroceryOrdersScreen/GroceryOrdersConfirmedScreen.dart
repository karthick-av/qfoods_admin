...
class GroceryOrdersConfirmedScreen extends StatefulWidget {
  const GroceryOrdersConfirmedScreen({super.key});

  @override
  State<GroceryOrdersConfirmedScreen> createState() => _GroceryOrdersConfirmedScreenState();
}

class _GroceryOrdersConfirmedScreenState extends State<GroceryOrdersConfirmedScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   if((orders?.confirmed?.length ?? 0) == 0 && orders?.lconfirmed == false){
...
try{
       String url = "${ApiServices.grocery_get_orders}?status=2&page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
    var response = await http.get(Uri.parse(url));
   orders.updateConfirmed();
          setState(() {
    ApiCallDone = false;
  });
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      orders.addAllConfirmed(__orders);   
      }else{
       orders.addConfirmed(__orders);   
...
getOrdersHandler("init", val);
  ...
  Widget build(BuildContext context) {
    ...
    final orders = Provider.of<GroceryOrdersProvider>(context, listen: true).confirmed;
