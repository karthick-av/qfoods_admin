...
class ViewTagsDishesScreen extends StatefulWidget {
  final TagsModel tag;
  const ViewTagsDishesScreen({super.key, required this.tag});

  @override
  State<ViewTagsDishesScreen> createState() => _ViewTagsDishesScreenState();
}

class _ViewTagsDishesScreenState extends State<ViewTagsDishesScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   final dishesProvider = Provider.of<TagsDishesProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.get_tags}/dishes/${widget.tag?.tagId}?page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
Future<void> AdditemHandler(List<CategoryDishesModel> dishes) async{
     try{
       String url = "${ApiServices.get_tags}/dishes/";
     List<dynamic> data = [];
     dishes?.forEach((e) {
      data.add({
...
Future<void> deleteitemHandler(CategoryDishesModel dish) async{
  final dishesProvider = Provider.of<TagsDishesProvider>(context, listen: false);
     try{
       String url = "${ApiServices.get_tags}/dishes/${widget?.tag?.tagId}/${dish?.dishId}";
   print(url);
 ...
 final dishesProvider = Provider.of<TagsDishesProvider>(context, listen: true);
...
class TagsDishesProvider extends ChangeNotifier{
