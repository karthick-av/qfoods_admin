...
import 'package:admin/components/GroceryTypeForm.dart';
...
class GroceryTypesScreen extends StatefulWidget {
  const GroceryTypesScreen({super.key});

  @override
  State<GroceryTypesScreen> createState() => _GroceryTypesScreenState();
}

class _GroceryTypesScreenState extends State<GroceryTypesScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
    final list = Provider.of<GroceryTypesProvider>(context, listen: false);

   if((list?.list?.length ?? 0) == 0){
      getTypesHandler("init", "");
     ...
     scrollController.addListener(() { 
      ...
      print(ApiCallDone);
       ...
       if(!ApiCallDone){
       getTypesHandler("bottom",filterCondition);
...
Future<void> getTypesHandler(String type, String filter)async {
  if(CompleteAPI) return;
final typesProvider = Provider.of<GroceryTypesProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.get_grocery_type}?page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
setState(() {
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      typesProvider.addAll(_list);   
      }else{
       typesProvider.add(_list);   
...
Future<void> deleteHandler(int id) async{
 loading = true;
  ...
  try{
      ...
      var response = await http.delete(Uri.parse("${ApiServices.get_grocery_type}/${id}"));
  loading = false;
    setState(() {});
    
   if(response.statusCode == 200){
    typesProvider.remove(id);
  }
  }
  catch(e){
      loading = false;
...
final typesProvider = Provider.of<GroceryTypesProvider>(context, listen: true);
      ...
      body: RefreshIndicator(
           ...
           padding: const EdgeInsets.symmetric(horizontal: 15,vertical: 10),
                           ...
                           child: TextField(
                            ...
                            ResetState();
                await getTypesHandler("init", "search=${val}");
                    ...
                    child: Column(
                      children: [
                         ...
                         ListView.builder(
                          ...
                          itemCount: typesProvider?.list?.length ?? 0,
                          itemBuilder: (BuildContext context, int index){
                            final data = typesProvider?.list;
                           return InkWell(
                            onTap: (){
              Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => 
                        ViewGroceryTypeProductsScreen(type: data![index],)),
                        );
                            },
                             child: Container(
...
children:  [
      ...
      SlidableAction(
       ...
       UpdateGroceryTypeForm(context, data![index]);
...
children:  [
      ...
      SlidableAction(
         onPressed: ((context) {
        deleteHandler(data?[index]?.typeId ?? 0);
        }),
        ...
        label: 'Delete',
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(width: 10,),
                                    Text(data?[index].typeName ?? '', style: TextStyle(color: AppColors.blackcolor, fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(14), fontWeight: FontWeight.w400),)
                                  ],
                                ),
                              )
                             ),
...
class GroceryTypesProvider extends ChangeNotifier{
 List<Types> list = [];
  addAll(List<Types> data) {
    list = data;
    notifyListeners();
  }
  ...
  addNew(Types data) {
    int? index = list?.indexWhere((e) => (e?.typeId ?? 0) == data?.typeId);
    if(index == -1){
      list.insert(0, data);
    }
 ...
 Update(Types data) {
    int index = list?.indexWhere((e) => (e?.typeId ?? 0) == data?.typeId) ?? -1;
    if(index != -1){
      list[index] = data;
    }
  ...
  remove(int id) {
   list.removeWhere((e) => e?.typeId == id);
  }
