...
class ViewGroceryTypeProductsScreen extends StatefulWidget {
  final Types type;
  const ViewGroceryTypeProductsScreen({super.key, required this.type});

  @override
  State<ViewGroceryTypeProductsScreen> createState() => _ViewGroceryTypeProductsScreenState();
}

class _ViewGroceryTypeProductsScreenState extends State<ViewGroceryTypeProductsScreen> {
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   ...
   final productsprovider = Provider.of<GroceryTypeProductsProvider>(context, listen: false);
   if((productsprovider?.list?.length ?? 0) == 0){
      getTypeProductHandler("init", "");
     ...
     scrollController.addListener(() { 
      ...
      print(ApiCallDone);
       ...
       if(!ApiCallDone){
       getTypeProductHandler("bottom",filterCondition);
...
Future<void> getTypeProductHandler(String type, String filter)async {
   final productsProvider = Provider.of<GroceryTypeProductsProvider>(context, listen: false);
...
try{
       String url = "${ApiServices.get_grocery_type}/products/${widget.type?.typeId}?page=${current_page}&per_page=${per_page}&${filter}";
     print(url);
...
filterCondition = val;
setState(() {});
getTypeProductHandler("init", val);
}
...
Future<void> AdditemHandler(List<SearchProductModel> product) async{
     try{
       String url = "${ApiServices.get_grocery_type}/products/";
     List<dynamic> data = [];
     product?.forEach((e) {
      data.add({
        "product_id": e?.groceryId?.toString(),
        "type_id":  widget?.type?.typeId?.toString()
      });
     });
 var header ={
...
Future<void> deleteitemHandler(SearchProductModel product) async{
  final productsProvider = Provider.of<GroceryTypeProductsProvider>(context, listen: false);
     try{
       String url = "${ApiServices.get_grocery_type}/products/${widget?.type?.typeId}/${product?.groceryId}";
   print(url);
 ...
 final productsProvider = Provider.of<GroceryTypeProductsProvider>(context, listen: true);
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
             onRefresh: () async{
              ResetState();
                await getTypeProductHandler("init", "");
           ...
           padding: const EdgeInsets.symmetric(horizontal: 15,vertical: 10),
                           ...
                           child: TextField(
                            ...
                            ResetState();
                await getTypeProductHandler("init", "search=${val}");
...
class GroceryTypeProductsProvider extends ChangeNotifier{
