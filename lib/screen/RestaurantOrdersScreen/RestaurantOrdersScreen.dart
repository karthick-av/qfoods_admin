...
import 'package:admin/screen/RestaurantOrdersScreen/ViewOrderScreen.dart';
...
void initState(){
  ...
  WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
   if((ordersController?.orders?.length ?? 0) == 0){
...
try{
       String url = "${ApiServices.get_orders}?page=${current_page}&per_page=${per_page}&${filter}";
   print(url);
...
setState(() {
    ...
    if(response.statusCode == 200){
      ...
      __orders.add(OrderModel.fromJson(json));
       }
    ...
    current_page = current_page + 1;
      ...
      if(type == "init"){
      ordersController.addAllOrders(__orders);   
      }else{
       ordersController.addOrders(__orders);   
  ...
  Widget build(BuildContext context) {
      ...
      drawer: DrawerMenu(),
        appBar: AppBar(
          ...
          child: Row(
            ...
            children: [
             Icon(Icons.filter_alt_outlined, size: ScreenUtil().setSp(24), color: AppColors.whitecolor,)
            ,    SizedBox(width: 10,),
            ],
          ),
        )
   ...
   iconTheme: IconThemeData(
    color: Colors.white, // <-- SEE HERE
        ...
        elevation: 0.5,
        title: Text("My Orders", style: TextStyle(fontFamily: FONT_FAMILY, fontSize: ScreenUtil().setSp(16.0), fontWeight: FontWeight.bold,color: AppColors.whitecolor),),
      ),
      body: SafeArea(
            ...
            child: Column(
              ...
              ListView.builder(
               physics: NeverScrollableScrollPhysics(),
                shrinkWrap: true,
                itemCount: ordersController?.orders?.length ?? 0,
